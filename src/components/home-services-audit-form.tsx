...
export function HomeServicesAuditForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      source: "clearautomations-industries-home-services",
      ...
      serviceArea: (form.elements.namedItem("serviceArea") as HTMLInputElement)
        .value,
      subVertical: (
        form.elements.namedItem("subVertical") as HTMLSelectElement
      ).value,
      monthlyLeadVolume: (
      ...
      biggestLeak: (form.elements.namedItem("biggestLeak") as HTMLSelectElement)
        .value,
      licenseInfo: (form.elements.namedItem("licenseInfo") as HTMLInputElement)
        .value,
      dataAcknowledgement: (
    ...
    return (
      <div className="py-8 text-center">
        ...
        <h3 className="font-[var(--font-display)] text-[1.4rem] font-bold text-white mb-2">
          Lead-funnel audit requested.
        </h3>
        <p className="text-[var(--navy-text)] text-[15px] leading-[1.7]">
          You will receive a sub-vertical-specific lead-funnel review within 2
          business days.
        </p>
      </div>
    );
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Service area" className="sm:col-span-2">
          <input type="text" name="serviceArea" placeholder="DFW, San Antonio, Austin, etc." required className={inputClass} />
        </Field>
        <Field label="Sub-vertical">
          <select name="subVertical" required defaultValue="" className={selectClass}>
            <option value="" disabled>Sub-vertical</option>
            ...
            <option value="other">Other (sub-vertical script customization add-on)</option>
          </select>
        </Field>
        <Field label="Monthly lead volume">
          <select name="monthlyLeadVolume" required defaultValue="" className={selectClass}>
            ...
            <option value="under-20">Under 20 leads</option>
        ...
        <Field label="Current FSM/CRM tools" className="sm:col-span-2">
          <input type="text" name="currentTools" placeholder="Jobber, Housecall Pro, ServiceTitan, FieldEdge, spreadsheet, etc." required className={inputClass} />
        </Field>
        <Field label="Biggest revenue leak" className="sm:col-span-2">
          <select name="biggestLeak" required defaultValue="" className={selectClass}>
            ...
            <option value="estimate-followup">Unfollowed estimates</option>
            <option value="recurring-renewals">Recurring service renewals (pest, landscape)</option>
            <option value="reviews">Not enough reviews</option>
            <option value="unclear">Not sure yet</option>
          </select>
        </Field>
        <Field label="State license number (optional, plumbing/electrical/pest)" className="sm:col-span-2">
          <input type="text" name="licenseInfo" placeholder="e.g., TX TSBPE M-12345 (optional)" className={inputClass} />
        </Field>
      </div>
      ...
      <label className="flex gap-3 items-start text-[13px] leading-[1.6] text-[var(--navy-text)]">
        ...
        <span>
          ...
          will not include customer payment card details, access codes, gate
          codes, or anything that should stay inside our FSM.
        </span>
      </label>
      <ContactConsent>
      ...
      >
        {status === "submitting" ? "Sending..." : "Request Lead-Funnel Audit"}
      </button>
