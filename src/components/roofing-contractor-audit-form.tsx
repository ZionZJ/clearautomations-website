...
export function RoofingContractorAuditForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      source: "clearautomations-industries-roofing-contractors",
      ...
      stormLeadSource: (
        form.elements.namedItem("stormLeadSource") as HTMLSelectElement
      ).value,
    ...
    return (
      <div className="py-8 text-center">
        ...
        <h3 className="font-[var(--font-display)] text-[1.4rem] font-bold text-white mb-2">
          Roofing workflow audit requested.
        </h3>
        <p className="text-[var(--navy-text)] text-[15px] leading-[1.7]">
          You will receive a storm-lead and inspection-booking workflow review
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Service area" className="sm:col-span-2">
          <input type="text" name="serviceArea" placeholder="DFW, Austin, Houston, etc." required className={inputClass} />
        </Field>
        ...
        <Field label="Primary storm lead source">
          <select name="stormLeadSource" required defaultValue="" className={selectClass}>
            <option value="" disabled>Storm lead source</option>
            <option value="google-lsa">Google LSA/search</option>
            <option value="referrals">Referrals</option>
            <option value="door-knocking">Door knocking</option>
            <option value="facebook">Facebook/social</option>
            <option value="insurance-agent">Insurance agent referrals</option>
            <option value="mixed">Mixed sources</option>
          </select>
        </Field>
        <Field label="Current CRM/roofing tools" className="sm:col-span-2">
          <input type="text" name="currentTools" placeholder="JobNimbus, AccuLynx, HubSpot, spreadsheet, etc." required className={inputClass} />
        </Field>
        <Field label="Biggest revenue leak" className="sm:col-span-2">
          <select name="biggestLeak" required defaultValue="" className={selectClass}>
            ...
            <option value="storm-lead-speed">Slow storm lead response</option>
            <option value="missed-calls">Missed calls</option>
            <option value="inspection-scheduling">Inspection scheduling</option>
            <option value="claim-followup">Insurance claim follow-up</option>
            <option value="unsigned-contracts">Unsigned estimates/contracts</option>
      ...
      <label className="flex gap-3 items-start text-[13px] leading-[1.6] text-[var(--navy-text)]">
        ...
        <span>
          ...
          will not include payment card details, access codes, claim documents,
          private homeowner notes, or information that should stay inside our
          production system.
        </span>
      </label>
      <ContactConsent>
      ...
      >
        {status === "submitting"
          ? "Sending..."
          : "Request Roofing Workflow Audit"}
      </button>
