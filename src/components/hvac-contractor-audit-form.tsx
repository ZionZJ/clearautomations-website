...
export function HvacContractorAuditForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const data = {
      ...
      source: "clearautomations-industries-hvac-contractors",
    ...
    return (
      <div className="py-8 text-center">
        ...
        <h3 className="font-[var(--font-display)] text-[1.4rem] font-bold text-white mb-2">
          HVAC workflow audit requested.
        </h3>
        <p className="text-[var(--navy-text)] text-[15px] leading-[1.7]">
          You will receive a missed-call and emergency-booking workflow review
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Field label="Current CRM/dispatch tools" className="sm:col-span-2">
          <input type="text" name="currentTools" placeholder="ServiceTitan, Jobber, HubSpot, spreadsheet, etc." required className={inputClass} />
        </Field>
        <Field label="Biggest revenue leak" className="sm:col-span-2">
          <select name="biggestLeak" required defaultValue="" className={selectClass}>
            ...
            <option value="maintenance-renewals">Maintenance renewals</option>
      ...
      >
        {status === "submitting" ? "Sending..." : "Request HVAC Workflow Audit"}
      </button>
